...
(function(){
    ...
    <nav class="border-b border-gray-200 dark:border-gray-700 py-4 sticky top-0 bg-white/90 dark:bg-[#202124]/90 backdrop-blur-sm z-50">
        ...
        <div id="mobileMenu" class="hidden md:hidden bg-white dark:bg-[#202124] mt-4">
            <a href="${yayHref}" class="block text-sm font-medium text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 px-6 py-3 transition-colors">Streamer İçin</a>
    ...
    const desktopLinks = container.querySelectorAll('#desktopNav .nav-link');
    if(inYay){
        desktopLinks.forEach(a=>{ if(a.textContent.trim().includes('Streamer')) a.classList.add('active'); });
    }
